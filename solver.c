...
void solver_diffuse ( fluid_solver* solver, uint32_t b, float * x, float * x0){
	...
	solver_lin_solve_asm ( solver, b, x, x0, a, 1+4*a );
}
...
void solver_project_c ( fluid_solver* solver, float * p, float * div ){
	...
	FOR_EACH_CELL
		...
		p[IX(i,j)] = 0;
	END_FOR
	solver_set_bnd_asm ( solver, 0, div ); solver_set_bnd_asm ( solver, 0, p );
	solver_lin_solve_asm ( solver, 0, p, div, 1, 4 );
	FOR_EACH_CELL
		solver->u[IX(i,j)] -= 0.5f*solver->N*(p[IX(i+1,j)]-p[IX(i-1,j)]);
		solver->v[IX(i,j)] -= 0.5f*solver->N*(p[IX(i,j+1)]-p[IX(i,j-1)]);
	END_FOR
	solver_set_bnd_asm ( solver, 1, solver->u ); solver_set_bnd_asm ( solver, 2, solver->v );
}
